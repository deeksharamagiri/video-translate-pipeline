...
def split_sentences(paragraph: str, lang: str) -> List[str]:
    ...
    """
    # mosestokenizer/nltk are only needed for this paragraph-splitting path
    # (unused by Model.batch_translate, which is all this project calls —
    # segments already arrive pre-split from ASR/Stage 3), so import lazily
    # rather than requiring them for every Krutrim-Translate call.
    if lang == "eng_Latn":
...
class Model:
    ...
    def __init__(
        ...
        Args:
            ...
            compute_type (str, optional): ctranslate2 compute type. "float16"
                only runs on GPU; defaults to "int8" on cpu, "float16" on cuda.
        """
        ...
        if model_type == "ctranslate2":
            ...
            self.translator = ctranslate2.Translator(
                self.ckpt_dir, device=device, compute_type=compute_type
            )
